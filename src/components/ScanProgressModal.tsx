...
export const ScanProgressModal = ({ open, scanType, onClose, autoCloseDelayMs = 1500 }: ScanProgressModalProps) => {
  const [currentStage, setCurrentStage] = useState(0)
  const [progress, setProgress] = useState(0)
  ...
  }, [progress, open, onClose, autoCloseDelayMs])
      ...
      <DialogContent className="glass-card border-primary/20 max-w-md [&>button]:hidden">
        <div className="space-y-6 py-4">
          ...
          <div className="text-center space-y-2">
            ...
            <p className="text-sm text-muted-foreground">Please wait while we analyze the target...</p>
          </div>
          ...
          <div className="space-y-2">
            {stages.map((stage, index) => (
              ...
              >
                <div
                  className={`transition-colors ${index <= currentStage ? "text-primary" : "text-muted-foreground"}`}
                >
                  {stage.icon}
                </div>
                <span
