...
import { demoMessages } from "@/lib/demoData"
import { toast } from "sonner"
import { sendMessageToGemini, type Message } from "@/lib/geminiService"
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <header className="border-b border-border bg-card/50 backdrop-blur-sm sticky top-0 z-50">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button variant="ghost" size="icon" onClick={() => setInspectorOpen(!inspectorOpen)} className="md:hidden">
              {inspectorOpen ? <X className="w-5 h-5" /> : <Menu className="w-5 h-5" />}
            </Button>
          </div>
        </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        {/* Inspector Panel */}
        <div
          className={`w-full md:w-[400px] lg:w-[500px] border-l border-border bg-card/30 backdrop-blur-sm ${inspectorOpen ? "flex" : "hidden md:flex"} flex-col`}
        >
          <InspectorPanel report={currentReport} messages={messages} scannedUrl={lastScannedUrl} />
        </div>
      </div>

      {/* Modals */}
      <URLInputModal open={urlModalOpen} onOpenChange={setUrlModalOpen} onSubmit={handleScanSubmit} />

      <ScanProgressModal
        open={!!scanningUrl}
        scanType={scanType}
        onClose={handleScanProgressClose}
